...
def metis(W, levels, rid=None):
    ...
    parents = []
    degree = W.sum(axis=0) - W.diagonal()  # Note that this sums the weights
    print(degree)
    graphs = []
    graphs.append(W)
    ...
    for _ in range(levels):
        ...
        # Compute the clusterino
        cluster_id = metis_one_level(W.row, W.col, W.data, rid, weights)
        ...
        nrr = cluster_id[W.row]
        print(nrr)
        ncc = cluster_id[W.col]
        print(ncc)
        nvv = W.data
        print(nvv)
        n_nodes = cluster_id.max() + 1
        shape = (n_nodes, n_nodes)
        # CSR is more appropriate: row,val pairs appear multiple times
        W = scipy.sparse.csr_matrix((nvv,(nrr,ncc)), shape=shape)
        print(W)
        W.eliminate_zeros()
