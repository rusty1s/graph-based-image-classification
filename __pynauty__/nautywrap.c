...
static PyObject*
graph_canonical_labeling(PyObject *self, PyObject *args)
{
    PyObject *py_graph;
    NyGraph * g;
    PyObject *pylist;
    ...
    pylist = PyList_New(g->no_vertices);

    if (pylist != NULL) {
      int i;
      for (i = 0; i < g->no_vertices; i++) {
#if PY_MAJOR_VERSION >= 3
        PyList_SET_ITEM(pylist, i, PyLong_FromLong(g->lab[i]));
#else
        PyList_SET_ITEM(pylist, i, PyInt_FromLong(g->lab[i]));
#endif
      }
    }
...
static PyMethodDef nautywrap_methods[] = {
    ...
    {"graph_canonical_labeling", graph_canonical_labeling, METH_VARARGS, graph_canonical_labeling_docs},
